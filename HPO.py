...
# === Final-Precision HPO Parameters ===
L = 180.0
dz = 0.0015
kappa = 1000
beta = 0.1
...
# Target: first 30 Riemann zeros
num_tn = 30
true_tn = np.array([float(zetazero(n).imag) for n in range(1, num_tn + 1)])

# σ sweep: high resolution & high ceiling
sigma_list = list(range(200, 11600, 100))
extracted = set()

for sigma in sigma_list:
    try:
        eigvals, _ = eigsh(H, k=35, sigma=sigma, which='LM', tol=1e-10)
...
# Greedy match ζ zeros to closest √λₙ
matched = []
used = set()
for t in true_tn:
...
plt.plot(true_tn, true_tn, '--', label='Ideal y = x', color='gray')
plt.xlabel("Riemann zero tₙ")
plt.ylabel("√λₙ from HPO")
plt.title("HPO Eigenvalue Matching: First 30 Zeta Zeros (v5 Precision)")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig("hpo_match_first_30_v5.png")
plt.show()
